...
async function getCountriesBD(search) {
    ...
    let response = false;
    if (idPais) response = await Country.findByPk(idPais, { include: Activity }); //{ include: [{ model: Activity, through: { attributes: [] } }] })
...
async function getActivities() {
    ...
    const uniqueActivities = activities
        ...
        // .unshift("Todas");   // No es el mejor lugar para ponerlo, pero porque no funciona?
